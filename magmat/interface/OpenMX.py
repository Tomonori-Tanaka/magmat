...
class OpenmxParser(object):
    ...
    def load_initial_structure(self, file_original):
        ...
        with open(file_original, 'r') as f:
            ...
            for line in lines[ipos_coord:fpos_coord]:
                ...
                try:
                    moment_direction.append([float(t) for t in line_split[7:11]])
                    constraint.append(line_split[11])
                    orb_polar.append(line_split[12])
                except:
                    moment_direction.append([.0, .0, .0, .0])
                    constraint.append(1)
                    orb_polar.append('off')
        ...
        self._moment_direction = moment_direction
        self._constraint = constraint
        self._orb_polar = orb_polar
        self._kmesh = kgrid
    ...
    def generate_structures(self, prefix, header_list, disp_list):
        ...
        for header, disp in zip(header_list, disp_list):
            self.generate_input(prefix, header, disp)
    ...
    def generate_input(self, prefix, header, moment):
        moment = np.array(moment)
        self._prefix = prefix

        filename = self._prefix + str(header) + ".dat"
            ...
            for line in self._common_settings:
                ...
                if "atoms.number" in line.lower():
                    ...
                    for i in range(self._nat):
                        ...
                        x_cartesian_disp = np.dot(self._x_fractional[i], self._lattice_vector.transpose())
                        for j in range(3):
                            f.write("%21.14f" % x_cartesian_disp[j])
                        for j in range(2):
                            f.write("%6.2f" % (self._initial_charges[i, j]))
                        for j in range(4):
                            f.write("%12.6f" % moment[i, j])
                        f.write("%3s" % (self._constraint[i]))
                        f.write("%5s" % self._orb_polar[i])
                        f.write('\n')
                    f.write("Atoms.SpeciesAndCoordinates>\n")
    ...
    def _get_energies_outfile(out_file):
        ...
        return np.array(etot, dtype=np.double)
